...
import { settlePortEntry, portLevel, xpThreshold } from "@/game/domain/rules/progression";
describe("port progression", () => {
  it("settles only into a different port using persisted market reference", () => {
    ...
    expect(moved.xpGained).toBe(148);
    expect(moved.state.portProgress.lisbon.xp).toBe(148);
